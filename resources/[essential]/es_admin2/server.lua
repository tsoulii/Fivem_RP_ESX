			...
			TriggerEvent('es:canGroupTarget', user.getGroup(), groupsRequired[type], function(available)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "goto2" then TriggerClientEvent('es_admin:quick', Source, type, id) end
...
-- Freezing
local frozen = {}
TriggerEvent('es:addGroupCommand', 'goto2', "mod", function(source, args, user)
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			local player = tonumber(args[1])
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				if(target)then
					...
					TriggerClientEvent('es_admin:teleportUser', target, source.getCoords().x, source.getCoords().y, source.getCoords().z)
